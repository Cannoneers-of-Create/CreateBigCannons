...
public class IncompleteCannonBlockRecipe extends HardcodedBlockRecipe {
	...
	public IncompleteCannonBlockRecipe(Block result, Block start) {
		super(result);
	...
	public static Collection<IncompleteCannonBlockRecipe> makeAllIncompleteRecipes() {
		...
		.map(b -> new IncompleteCannonBlockRecipe(((IncompleteWithItemsCannonBlock) b).getCompleteBlockState(b.defaultBlockState()).getBlock(), b))
		.toList();
	}
