...
public class CannonMountScenes {
	...
	public static void assemblyAndUse(SceneBuilder scene, SceneBuildingUtil util) {
		...
		BlockPos cannonStart = new BlockPos(4, 5, 2);
		for (int i = 0; i < 5; ++i) {
		...
		scene.overlay.showText(80)
			...
			.pointAt(util.vector.blockSurface(new BlockPos(2, 2, 2), Direction.NORTH));
		scene.idle(90);
		...
		scene.addInstruction(CBCAnimateBlockEntityInstruction.cannonMountPitch(new BlockPos(2, 3, 2), 30, 40));
		...
		scene.addInstruction(CBCAnimateBlockEntityInstruction.cannonMountYaw(new BlockPos(2, 3, 2), -360, 200));
