...
public class ShrapnelSmokeParticle extends BaseAshSmokeParticle {

	private final Vec3 wind = ParticleWindHandler.getWindForce(0);

	ShrapnelSmokeParticle(ClientLevel level, double x, double y, double z, double dx, double dy, double dz, SpriteSet sprites) {
	...
	public static class Provider implements ParticleProvider<ShrapnelSmokeParticleData> {
		...
		@Override
		public Particle createParticle(ShrapnelSmokeParticleData data, ClientLevel level, double x, double y, double z, double dx, double dy, double dz) {
			ShrapnelSmokeParticle particle = new ShrapnelSmokeParticle(level, x, y, z, dx, dy, dz, this.sprites);
			particle.scale(6.0f);
			particle.setSize(0.35f, 0.35f);
