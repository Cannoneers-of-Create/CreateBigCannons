...
public class CBCTooltip {
	...
	public static <T extends Block & BigCannonBlock> void appendCannonBlockText(ItemStack stack, @Nullable Level level,
		...
		if (hasGoggles) {
			...
			tooltip.addAll(TooltipHelper.cutStringTextComponent(I18n.get(rootKey + ".strength.goggles", strength),
				palette.primary(), palette.highlight(), 2));
		} else {
			float nethersteelStrength = PowderChargeBlock.getPowderChargeEquivalent(
				CBCBigCannonMaterials.NETHERSTEEL.properties().maxSafeBaseCharges());
		...
		if (hasGoggles) {
			tooltip.addAll(TooltipHelper.cutStringTextComponent(
				I18n.get(rootKey + ".squibRatio.goggles", material.squibRatioNum(), material.squibRatioDem()),
				palette.primary(), palette.highlight(), 2));
		} else {
			double squibRatio = material.squibRatio();
			tooltip.add(
				getNoGogglesMeter(squibRatio < 1d ? 0 : Mth.ceil(material.squibRatio() * 5d / 3d), false, true));
		}
		...
		String failKey =
			material.failureMode() == BigCannonMaterialProperties.FailureMode.RUPTURE ? ".onFailure.rupture"
				: ".onFailure.fragment";
		tooltip.addAll(TooltipHelper.cutStringTextComponent(I18n.get(rootKey + failKey),
			TooltipHelper.Palette.GRAY_AND_WHITE.primary(), TooltipHelper.Palette.GRAY_AND_WHITE.highlight(), 1));

		float breechStrength = BigCannonBreechStrengthHandler.getStrength(block, material.maxSafeBaseCharges());
		tooltip.add(Component.literal(" " + I18n.get(rootKey + ".breechStrength")).withStyle(ChatFormatting.GRAY));
		tooltip.addAll(TooltipHelper.cutStringTextComponent(I18n.get(rootKey + ".breechStrength.desc", breechStrength),
			palette.primary(), palette.highlight(), 2));
	}

	public static <T extends Block & AutocannonBlock> void appendTextAutocannon(ItemStack stack, @Nullable Level level,
		List<Component> tooltip, TooltipFlag flag, T block) {
		boolean desc = Screen.hasShiftDown();
		addHoldShift(desc, tooltip);
		if (!desc) {
	...
	public static void appendMortarStoneText(ItemStack stack, @Nullable Level level, List<Component> tooltip,
		TooltipFlag flag) {
		...
		tooltip.addAll(TooltipHelper.cutStringTextComponent(I18n.get(key + ".value", value), palette.primary(),
			palette.highlight(), 1));
	}

	public static void appendRamRodText(ItemStack stack, @Nullable Level level, List<Component> tooltip,
		TooltipFlag flag) {
		if (!Screen.hasShiftDown()) {
			return;
		}
		...
		tooltip.addAll(TooltipHelper.cutStringTextComponent(I18n.get(key + ".value", RamRodItem.getPushStrength()),
		...
		tooltip.addAll(
			TooltipHelper.cutStringTextComponent(I18n.get(key1 + ".value", RamRodItem.getReach()), palette.primary(),
				palette.highlight(), 1));

		String key2 = keyBase + "deployerCanUse";
		tooltip.add(Component.literal(I18n.get(key2)).withStyle(ChatFormatting.GRAY));
		tooltip.addAll(
			TooltipHelper.cutStringTextComponent(I18n.get(key2 + (RamRodItem.deployersCanUse() ? ".yes" : ".no")),
				palette.primary(), palette.highlight(), 1));
	}

	public static void appendWormText(ItemStack stack, @Nullable Level level, List<Component> tooltip,
		TooltipFlag flag) {
		if (!Screen.hasShiftDown()) {
			return;
		}
		...
		tooltip.add(Component.literal(I18n.get(key)).withStyle(ChatFormatting.GRAY));
		tooltip.addAll(
			TooltipHelper.cutStringTextComponent(I18n.get(key + ".value", WormItem.getReach()), palette.primary(),
				palette.highlight(), 1));

		String key1 = keyBase + "deployerCanUse";
		tooltip.add(Component.literal(I18n.get(key1)).withStyle(ChatFormatting.GRAY));
		tooltip.addAll(
			TooltipHelper.cutStringTextComponent(I18n.get(key1 + (WormItem.deployersCanUse() ? ".yes" : ".no")),
				palette.primary(), palette.highlight(), 1));
	}

	public static void appendImpactFuzeText(ItemStack stack, @Nullable Level level, List<Component> tooltip,
		TooltipFlag flag, float detChance) {
		if (!Screen.hasShiftDown()) {
			return;
		}
		TooltipHelper.Palette palette = getPalette(level, stack);
		...
		tooltip.add(Component.literal(I18n.get(key)).withStyle(ChatFormatting.GRAY));
		tooltip.addAll(TooltipHelper.cutStringTextComponent(I18n.get(key + ".value", (int) (detChance * 100.0f)),
			palette.primary(), palette.highlight(), 1));
	}

	public static void appendCannonCarriageText(ItemStack stack, @Nullable Level level, List<Component> tooltip,
		TooltipFlag flag, BlockItem block) {
		if (!Screen.hasShiftDown()) {
			return;
		}
		TooltipHelper.Palette palette = getPalette(level, stack);
		...
		tooltip.addAll(
			TooltipHelper.cutStringTextComponent(I18n.get(key + ".fireCannon"), palette.primary(), palette.highlight(),
				1));

		String pitchMode = I18n.get(CBCClientCommon.PITCH_MODE.getTranslatedKeyMessage().getString());
		tooltip.addAll(
			TooltipHelper.cutStringTextComponent(I18n.get(key + ".keyPressed", pitchMode), primary, highlight));
		tooltip.addAll(
			TooltipHelper.cutStringTextComponent(I18n.get(key + ".pitchMode"), palette.primary(), palette.highlight(),
				1));
	}

	public static void appendMuzzleVelocityText(ItemStack stack, @Nullable Level level, List<Component> tooltip,
		TooltipFlag flag,
		...
		tooltip.addAll(
			TooltipHelper.cutStringTextComponent(I18n.get(key + ".value", s), palette.primary(), palette.highlight(),
				1));
	}

	public static void appendPropellantStressText(ItemStack stack, @Nullable Level level, List<Component> tooltip,
		TooltipFlag flag,
		BigCannonPropellantBlock propellant) {
		if (!Screen.hasShiftDown()) {
			return;
	...
	public static void appendPropellantPowerText(ItemStack stack, @Nullable Level level, List<Component> tooltip,
		TooltipFlag flag,
		BigCannonPropellantBlock propellant) {
		if (!Screen.hasShiftDown()) {
			return;
		...
		tooltip.addAll(TooltipHelper.cutStringTextComponent(I18n.get(key + ".value", min, max), palette.primary(),
			palette.highlight(), 1));
	}
