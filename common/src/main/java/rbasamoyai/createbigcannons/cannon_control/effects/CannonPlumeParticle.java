	...
	public void tick() {
		...
		for (int i = 0; i < count; ++i) {
			...
			if (this.plumesSetting == PlumeSetting.LEGACY) {
				...
				this.level.addParticle(new CannonSmokeParticleData(this.scale, new Vector3f(0.85f, 0.85f, 0.85f), new Vector3f(0.75f, 0.75f, 0.75f), 60),
			...
			if (this.plumesSetting == PlumeSetting.LEGACY) {
				...
				for (int i = 0; i < Mth.ceil(this.scale * 2); ++i) {
					Vec3 vel = this.direction.scale(0.5)
							...
							.scale(1);
					this.level.addParticle(new CannonSmokeParticleData(this.scale * 0.25f, new Vector3f(1.0f, 96f / 255f, 0.0f), new Vector3f(0.92f, 0.92f, 0.92f), 20),
						true, this.x, this.y, this.z, vel.x, vel.y, vel.z);
				}
			}
