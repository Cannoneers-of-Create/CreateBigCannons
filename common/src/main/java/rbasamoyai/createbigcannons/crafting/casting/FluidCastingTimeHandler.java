...
public class FluidCastingTimeHandler {
		...
		protected void apply(Map<ResourceLocation, JsonElement> map, ResourceManager resources, ProfilerFiller profiler) {
			...
			for (Map.Entry<ResourceLocation, JsonElement> entry : map.entrySet()) {
				...
				if (loc.getPath().startsWith("tags/")) {
					TagKey<Fluid> tag = TagKey.create(BuiltInRegistries.FLUID.key(), new ResourceLocation(loc.getNamespace(), loc.getPath().substring(5)));
					TAGS_TO_LOAD.put(tag, castingTime);
				} else {
					Fluid fluid = BuiltInRegistries.FLUID.getOptional(loc).orElseThrow(() -> {
	...
	public static void loadTags() {
		...
		for (Map.Entry<TagKey<Fluid>, Integer> entry : TAGS_TO_LOAD.entrySet()) {
			...
			for (Holder<Fluid> holder : BuiltInRegistries.FLUID.getTagOrEmpty(entry.getKey())) {
	...
	public static void writeBuf(FriendlyByteBuf buf) {
		...
		for (Map.Entry<Fluid, Integer> entry : FLUID_MAP.entrySet()) {
			buf.writeResourceLocation(BuiltInRegistries.FLUID.getKey(entry.getKey())).writeVarInt(entry.getValue());
		}
	...
	public static void readBuf(FriendlyByteBuf buf) {
		...
		for (int i = 0; i < sz; ++i) {
			...
			Optional<Fluid> op = BuiltInRegistries.FLUID.getOptional(id);
